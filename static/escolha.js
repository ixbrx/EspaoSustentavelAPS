...
document.getElementById('calcular-compensacao').addEventListener('click', () => {
    const creditosGerados = parseInt(document.getElementById('credito-quantidade').value);
    
    if (!isNaN(creditosGerados) && creditosGerados > 0) {
        // Mostrar opções de compensação
        document.getElementById('opcoes-compensacao').classList.remove('hidden');

        // Armazenar créditos gerados para uso posterior
        document.getElementById('opcoes-compensacao').setAttribute('data-creditos', creditosGerados);
    } else {
        alert('Por favor, insira um número válido de créditos de carbono.');
    }
});

// Evento de click nos botões de selecionar compensação
document.querySelectorAll('.selecionar').forEach(button => {
    button.addEventListener('click', (e) => {
        const custo = parseInt(e.target.parentElement.getAttribute('data-custo'));
        const creditosGerados = parseInt(document.getElementById('opcoes-compensacao').getAttribute('data-creditos'));
        
        if (creditosGerados >= custo) {
            alert(`Você selecionou a compensação de ${e.target.parentElement.querySelector('h4').innerText}.`);
            // Subtrair o custo do crédito gerado
            document.getElementById('opcoes-compensacao').setAttribute('data-creditos', creditosGerados - custo);
        } else {
            alert('Você não tem créditos suficientes para essa compensação.');
        }
    });
});
